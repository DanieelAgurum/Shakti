...
async function generarLinkSeguro(idPublicacion) {
  ...
  return `${window.location.origin}/Vista/usuaria/foro?publicacion=${hash}`;
}
...
async function compartirWhatsapp() {
  ...
  const texto = encodeURIComponent("¡Mira esta publicación!\n\n " + url);
...
async function compartirTwitter() {
  ...
  const texto = encodeURIComponent(
    "Revisa esta publicación que encontré en Shakti:"
  );
  window.open(
